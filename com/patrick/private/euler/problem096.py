...
from com.patrick.private.utils.sudoku_solver import Sudoku
...
class Problem96(object):
    ...
    def __init__(self, filename: str, rows: int = 9, columns: int = 9):
        self.sudokus = []
        with (open(filename) as file):
            grids = [line.strip() for line in file]
            grids = [grids[(rows + 1) * n:(rows + 1) * (n + 1)] for n in range(len(grids) // (rows + 1))]

        self.sudokus = [Sudoku(grid=grid, rows=rows, columns=columns) for grid in grids]

        print("Problem 96: Find the sum of the 3-digit numbers found in the top left corner of each solution grid.")
    ...
    def attempt_1(self):
        ...
        for sudoku in self.sudokus:
            sudoku.solve()
            result += reduce(lambda x, y: 10 * x + y, sudoku.grid[0][:3])
...
if __name__ == '__main__':
    problem = Problem96(filename="../../../../resources/problem096.txt")
